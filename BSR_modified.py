import base64, time, numpy as np, tabula, pandas as pd, operator as op, sys, json, os
...
def bank_statement_read(file_path, bank_name):
    ...
    df_list = df
    col_list=['Description','Narration','Remarks','Particulars','DESCRIPTION','Description','Description',
                'PARTICULARS','Transaction Details','Narration','NARRATION','PERTICULERS','Details of transaction',
                'ransaction Remarks','Transaction\rParticulars','Transaction Description','ransaction Remarks',
                'Account Description','NARATION','Narration Chq/Ref No','Deposits','Amount (Rs.)','Credit',
                'CREDIT','Deposit Amt.','DEPOSIT','Deposit','Deposit Amount\r(INR )','Amount','Deposit Amt',
                'Type','Deposit Balance','WithDrawal','Value','Credit (Rs.)','Withdrawal Amt. Deposit Amt.',
                'Deposit Amount\r(INR )','Credit(Rs)','Withdrawals Deposits','Cr Amount','Withdrawal (Dr)/',
                'Withdrawal Amt.','Withdrawal','Withdrawal','DEBIT','Withdrawals','Debit','Withdrawal (Dr)',
                'Withdrawal Amt. Deposit Amt.','Withdrawal (Dr)/','Withdrawal Amount\r(INR )','Debit (Rs.)',
                'Debit(Rs)','WITHDRAWALS','Withdrawal INR','Withdrawal Amt','Amount','Withdrawal Amount',
                'Withdrawals Deposits','Dr Amount','Balance','Closing Balance','BALANCE','Running Balance',
                'BALANCE','Balance (INR )','Amount','MODE**','Running','Balance(Rs)','Total Amount\rDr/Cr',
                'Date','date','Transaction','Txn','Transaction Date','Debit/Credit','Transaction date','Txn Date',
                'Tran Date','Balance (?)','Branch','Cheque No','Chq./Ref.','Entry Date','Cheque',
                'TRANSACTION\rDATE','Trans Date','Tran Date','DATE','Post Date','Value\rDate','Date (Value\rDate)',
                'Tran Date','Txn dt','Txn Dt','Value Date','Chq./Ref.No.','Value Dt','Chq\rNo.',
                'Transaction Description','Total Amount\rDr/Cr','Debit (Rs.)','Credit (Rs.)','Balance (Rs.)',
                'Dr Amount','Cr Amount','Instruments','Total Amou']
    
    date_list=['Date','date','Transaction','Txn','Transaction Date','Transaction date','Txn Date','Tran Date',
                'Date','TRANSACTION\rDATE','Trans Date','Tran Date','DATE','Post Date','Value\rDate',
                'Date (Value\rDate)','Txn dt','Txn Dt','Value Date']
    
    narration_list=['Description','Narration','Remarks','Particulars','DESCRIPTION','Description',
                'Description','PARTICULARS','Transaction Details','Narration','NARRATION','PERTICULERS',
                'Details of transaction','ransaction Remarks','Transaction\rParticulars','Transaction Description',
                'ransaction Remarks','Account Description','NARATION','Narration Chq/Ref No']
    
    credit_list=['Deposits','Amount (Rs.)','Credit','CREDIT','Deposit Amt.','DEPOSIT','Deposit','Deposit Amount\r(INR )',
                'Amount','Credit (Rs.)','Withdrawal Amt. Deposit Amt.','Deposit Amount\r(INR )','Credit(Rs)','Withdrawals Deposits',
    ...
    debit_list=['Withdrawal Amt.','Withdrawal','Withdrawal','DEBIT','Withdrawals','Debit','Withdrawal (Dr)',
                'Withdrawal Amt. Deposit Amt.','Withdrawal (Dr)/','Withdrawal Amount\r(INR )','Debit (Rs.)',
                'Debit(Rs)','WITHDRAWALS','Withdrawal INR','Withdrawal Amt','Amount','Withdrawal Amount',
                'Withdrawals Deposits','WithDrawal','Dr Amount']
    ...
    data_frame = data.to_json(); bankData = data.T; data_json = bankData.to_json(); upi_json = UPI.T.to_json(); sal_json = salary.T.to_json(); neft_json = NEFT.T.to_json(); imps_json = IMPS.T.to_json(); nach_json = NACH.T.to_json()
    json_list = [data_json,sal_json,upi_json,nach_json,neft_json,imps_json]; json_out = json_bankstatements(json_list)
