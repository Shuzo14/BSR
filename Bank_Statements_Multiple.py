def bank_stament_read(dir_path,bank_name):
    ...
    #getting the total number of files in the folder and their names
    total_files = len([entry for entry in os.listdir(dir_path) if os.path.isfile(os.path.join(dir_path, entry))])
    files = os.listdir(dir_path)
    pd.set_option('display.max_rows',None)
    pd.set_option('display.max_columns',None)
    
    
    #taking files one by one as input
    for file in files:
        pdf_file = dir_path +"/"+ file
...
#bank_name = "sbi_bank"
#dir_path= 'C:/Users/VSK/Desktop/Bank_Statement/Bank statement'
#bank_stament_read(dir_path,bank_name)       
...
def bankStatement():
    bs = { "dir_path": request.json['dir_path'], "bank_name":request.json['bank_name']}
    bank = bank_stament_read(bs["dir_path"],bs["bank_name"]) 
    return (bank)
